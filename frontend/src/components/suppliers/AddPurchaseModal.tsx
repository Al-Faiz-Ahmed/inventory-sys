...
import type { PurchaseEntryFormData } from '../../../../shared/types';
...
export default function AddPurchaseModal({ open, onOpenChange, supplierId, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; supplierId: number; onCreated: () => void; }) {
  ...
  const [items, setItems] = useState<Array<{ id: string; productId: string | null; productName: string; quantity: string; unitPrice: string }>>([
    { id: crypto.randomUUID(), productId: null, productName: '', quantity: '', unitPrice: '' },
  ]);
  ...
  const mutation = useMutation({
    mutationFn: async (payload: PurchaseEntryFormData) => purchasesApi.createPurchase(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['purchases', { supplierId }] });
    },
    onError: (e: any) => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    // enforce: each item must reference an existing product
    ...
    const payload: PurchaseEntryFormData = {
      supplierId,
      ...
      onSuccess: async (created: any) => {
        try {
          if (cleanItems.length > 0) {
            await Promise.all(
              cleanItems.map(it =>
                purchasesApi.createPurchaseItem(created.id, {
          ...
          setError(e?.message || 'Failed to create purchase items');
        }
      },
    });
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl w-full p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border sticky top-0 bg-background z-10">
          <DialogTitle className="text-xl font-semibold">Add Purchase</DialogTitle>
          ...
          <form onSubmit={onSubmit} className="px-6 py-6 space-y-4">
              ...
              <div className="col-span-12 md:col-span-7 space-y-2">
                ...
                <div className="relative">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => setForm((f) => ({ ...f, invoiceNumber: generateInvoiceId() }))}
            ...
            <div className="space-y-3">
                      ...
                      <tr key={it.id} className="align-middle">
                        <td className="p-2">
                          <Button
                            ...
                            onClick={() => setItems(prev => prev.length > 1 ? prev.filter(p => p.id !== it.id) : prev)}
                          ...
                          <div className="space-y-1 relative">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                // reset productId when typing free text
                                setItems(prev => prev.map(p => p.id === it.id ? { ...p, productName: v, productId: null } : p));
                              }}
                                ...
                                {productOptions
                                  ...
                                  .map(opt => (
                                    <button
                                      ...
                                      onClick={() => {
                                        setItems(prev => prev.map(p => p.id === it.id ? { ...p, productName: opt.name, productId: opt.id } : p));
                        ...
                        <td className="p-2 text-right">
                          <Input
                            ...
                            onChange={(e) => {
                              const v = e.target.value;
                              setItems(prev => prev.map(p => p.id === it.id ? { ...p, quantity: v } : p));
                            }}
                          />
                        </td>
                        <td className="p-2 text-right">
                          <Input
                            ...
                            onChange={(e) => {
                              const v = e.target.value;
                              setItems(prev => prev.map(p => p.id === it.id ? { ...p, unitPrice: v } : p));
                            }}
                          />
                        </td>
                        <td className="p-2 text-right">
              ...
              <div className="flex justify-between items-center">
                <Button
                  type="button"
                  onClick={() => setItems(prev => [...prev, { id: crypto.randomUUID(), productId: null, productName: '', quantity: '', unitPrice: '' }])}
                >
                  Add Item
                </Button>
                <div className="text-right">
                  ...
                  <div className="text-lg font-semibold">
                    {items.reduce((sum, it) => sum + Number(it.quantity || 0) * Number(it.unitPrice || 0), 0).toFixed(2)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <DialogFooter className="px-0 pt-2">
              ...
              <Button type="submit" disabled={submitting}>
                {submitting ? (<><Spinner className="mr-2 h-4 w-4" />Saving...</>) : 'Save'}
              </Button>
            </DialogFooter>
          </form>
        </div>
